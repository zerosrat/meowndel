...
async function select(key:string){
 ...
 try{const instance=scene=createComparisonScene(canvas,stage);const result=await instance.load('./'+asset.file,asset.rotationY??0);if(turn!==serial||!result)return;kind.textContent='可旋转 3D';angles.forEach(b=>b.disabled=false);status.textContent='拖动转身 · 双指或滚轮缩放';}
...
const readyCount=coats.filter(c=>ready.has(c.key)).length;
el('coverage').textContent=readyCount===coats.length?`${readyCount} 种花色均可旋转查看，点击下方预设切换。`:`${readyCount} 种可旋转模型；其余花色已完成参考图。`;
