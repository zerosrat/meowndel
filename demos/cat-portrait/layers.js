...
const master=canvas(), map=canvas(), mc=map.getContext('2d',{willReadFrequently:true});
let source, selected=4, finished=[];
...
function show(){
  const coat=coats[selected];
  const out=$('source').checked?master:render(coat,$('pattern').checked,$('white').checked);
  const ctx=$('portrait').getContext('2d');ctx.clearRect(0,0,N,N);ctx.drawImage(out,0,0);
  $('portrait').setAttribute('aria-label',`中华田园猫 · ${$('source').checked?'无花纹底稿':coat.name}`);
  $('caption').textContent=$('source').checked?'无花纹底稿':`中华田园猫 · ${coat.name}`;
  $('description').textContent=coat.desc;
  $('pattern').disabled=$('source').checked||!(coat.stripe||coat.patch);
  $('white').disabled=$('source').checked||!coat.white;
  document.querySelectorAll('[data-coat]').forEach(b=>b.setAttribute('aria-pressed',Number(b.dataset.coat)===selected));
  $('download').href=out.toDataURL('image/png');$('download').download=`meowndel-${$('source').checked?'neutral':coat.id}.png`;
}
const img=new Image();
img.onload=()=>{try{
  master.getContext('2d').drawImage(img,0,0,N,N);source=master.getContext('2d').getImageData(0,0,N,N);
  coats.forEach((coat,index)=>{
    const select=()=>{selected=index;show();};
    const button=document.createElement('button');button.textContent=coat.name;button.dataset.coat=index;button.onclick=select;$('coats').append(button);
    const card=document.createElement('button');card.className='card';card.dataset.coat=index;card.onclick=select;card.setAttribute('aria-label',`预览${coat.name}`);
    const preview=render(coat);finished.push(preview);preview.setAttribute('aria-hidden','true');card.append(preview,document.createTextNode(coat.name));$('grid').append(card);
  });
  let alphaErrors=0,eyeErrors=0;
  finished.forEach(out=>{const d=out.getContext('2d').getImageData(0,0,N,N).data;for(let i=3;i<d.length;i+=4)if(d[i]!==source.data[i])alphaErrors++;
    for(const [x,y]of [[394,304],[556,341],[393,287],[556,323],[457,360]]){const i=(y*N+x)*4;for(let c=0;c<4;c++)if(d[i+c]!==source.data[i+c])eyeErrors++;}
  });
  $('status').textContent=alphaErrors||eyeErrors?'校验发现差异，请检查底稿保护区域。':'已生成 6 种花色 · 轮廓透明度逐像素一致 · 五官采样点一致';
  $('status').dataset.alphaErrors=alphaErrors;$('status').dataset.featureErrors=eyeErrors;
  for(const id of ['pattern','white','source'])$(id).onchange=show;
  $('background').onclick=e=>{const b=e.target.closest('button');if(!b)return;$('stage').className=`stage ${b.dataset.bg}`;$('background').querySelectorAll('button').forEach(el=>el.setAttribute('aria-pressed',el===b));};
  show();
}catch(e){$('status').textContent=`生成失败：${e.message}`;$('status').className='error';}};
img.onerror=()=>{$('status').textContent='底稿加载失败，请刷新重试。';$('status').className='error';};
img.src='flat-assets/domestic-neutral-master.png';
